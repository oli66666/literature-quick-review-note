...
- Default pattern per point: **a bold short mechanism/key-phrase label**, followed by 1–2 concise sentences of elaboration — not a paragraph-length translation of the original. E.g. `**核心机制:被困感 + 无法摆脱的焦虑循环。** 参与者持续为月度支出与账单能否覆盖而担忧……`
...
2. `题录信息` (Heading 2) — a 2-column table, label column left, value column right:
   ...
   - DOI
   - 所属领域: **at most 3** most-relevant fields/keywords, chosen based on the paper's actual content
   - 论文类型: in **English only** (e.g. "Empirical, qualitative research article.")
   - 研究范式 / 数据收集方法 / 抽样策略 / 样本构成 / 理论框架 / 伦理 — each a tight paragraph
