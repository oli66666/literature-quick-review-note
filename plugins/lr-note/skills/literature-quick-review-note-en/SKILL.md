---
name: literature-quick-review-note-en
description: "Build a condensed \"quick overview\" intensive-reading note for an academic paper as a WPS-compatible Word (.docx) file, written entirely in English, with real margin comments grounded in genuine literature. Use when the user asks for a quick-overview / quick-review / condensed reading note (速览笔记) for a paper, as distinct from a full verbose note, AND either explicitly wants the note in English (in any request language, e.g. 做英文版速览笔记), or names no output language and writes the request in English. If the user explicitly asks for the note in Chinese, or writes in Chinese without naming a language, use literature-quick-review-note-cn instead."
metadata:
  version: 1.0.0
---
...
This is a **condensed variant** of intensive-reading notes: the whole paper's framework and precise content, distilled — not a paragraph-by-paragraph translation or restatement. Use it when the user asks for a "quick overview", "quick review", "condensed note" or "summary note" style note in English, or references wanting "this version" of a note style they've approved before. If the user just says "reading note" / "intensive-reading note" with no quick-overview signal, prefer a fuller methodology instead, if one is available. This skill always writes in English, regardless of what language the source paper is in.

**Output language, and which of the two sibling skills applies.** The sibling in the same plugin, `literature-quick-review-note-cn`, produces the same note in Chinese. Decide between them in this order: (1) if the user explicitly names an output language ("write the note in Chinese", "做英文版笔记"), follow it, whatever language the request itself is written in — so a Chinese-language request for an English note belongs here; (2) if they name none, follow the language of the request — English request, this skill; Chinese request, the `-cn` sibling. If the user invoked one of the two by name, use that one and do not second-guess it.

This skill folder is self-contained: `scripts/` ships portable Node.js and Python utilities for the document pipeline (build helpers, the WPS compatibility fix, comment insertion, validation, rendering). The `SKILL.md` instruction format is directly usable by Claude and Codex when installed as a compatible skill; other AI systems may require adapting the instruction file to their own format. The scripts themselves can be run independently, and are shared verbatim with the Chinese-output sibling skill — they manipulate OOXML and markup mechanics, not language content, so nothing about them is Chinese- or English-specific except one CLI default (see below).
...
2. **Every content-bearing heading gets the original PDF's page range in parentheses**, e.g. `1. Background and Research Questions (pp. 176–180)`. Look up real page numbers from the source PDF — never guess.
3. **No cross-paper comparison in the BODY of the note**, and no "connections to other literature in this project" section. The body reports what this one paper says and whom this one paper cites — nothing else.
...
4. **Body text is a distillation of the original, not a translation or restatement.** All content must be based on the real paper — never fabricate findings, citations, or numbers.

## Content style: summarize, don't restate at length

This is the part that distinguishes "quick overview" from a fuller note:

- Default pattern per point: **a bold short mechanism/key-phrase label**, followed by 1–2 concise sentences of elaboration — not a paragraph-length restatement of the original. E.g. `**Core mechanism: a sense of entrapment plus an unrelenting anxiety loop.** Participants describe continual worry over whether monthly income will cover fixed bills…`
...
- Cap direct quotes at roughly **one for the entire document**, used only where a quote is genuinely more illustrative than a paraphrase (only when a paraphrase would lose something the original wording actually carries). Paraphrase everything else.
...
1. Title line (Heading 1): `Literature Summary: <paper short title / theme> (pp. X–Y)`
2. `Bibliographic Information` (Heading 2) — a 2-column table, label column left, value column right:
   - Title: the paper's own title, as published, in its original language. **Only if that is not English**, add an English translation directly below it on its own line, **in gray text** (see Visual formatting). If the title is already in English, this is a single line — do not add a redundant second line.
   - Authors: bulleted list, one bullet per author with role/contribution in parentheses; then a `Note:` line with affiliation **in gray, ~2pt smaller than body** — no email addresses
   - Publication year: year only (no volume/issue, no "advance online publication" sentence — that's redundant)
   - Journal: a full **Harvard-style reference** (Author, A. (Year) 'Title', Journal, vol(issue), pp. x–y.) — journal-quality commentary (OA status, Scimago quartile/SJR/H-index) belongs in Impact Assessment instead, not duplicated here
   - DOI
   - Field(s): **at most 3** most-relevant fields/keywords, chosen based on the paper's actual content
   - Paper type: a short, unhedged genre label (e.g. "Empirical, qualitative research article.")
   - Research paradigm / Data collection method / Sampling strategy / Sample composition / Theoretical framework / Ethics — each a tight paragraph

   **Conceptual, review and theoretical papers.** The four empirical rows above (Research paradigm / Data collection method / Sampling strategy / Sample composition) assume a study with primary data. When the paper has none — a provocation paper, a narrative or systematic review, a purely theoretical piece — do not fill four rows with "not applicable"; that spends four rows of the table on zero information. Instead:
   - Collapse those four into a single row, `Empirical basis`, reading `Not applicable (no primary data)` plus one sentence naming where the evidence actually comes from (e.g. a narrative critique of the literature, drawing on existing empirical studies, policy documents and consultancy reports).
   - Give the reclaimed space to Theoretical framework: list every theory the paper applies OR stress-tests, since for this genre that list IS the substance.
   - Make Paper type explicit and unhedged: `Conceptual "provocation paper".` / `Narrative review article.` / `Systematic review.` / `Theoretical article.` — never leave it vague.
   - Ethics reads `Not applicable (no primary data)`, plus any conflict-of-interest or funding statement the paper does carry.
3. `Impact Assessment` (Heading 2) — bold-label bullets: Citation performance (via Crossref API lookup on the DOI) / Publication venue (via Scimago quartile/SJR/H-index lookup) / 2-4 Core contributions items / Methodological strengths / Citation caveats. One sentence may be `==highlighted==` (yellow) as the single standout takeaway — use sparingly, at most 1-2 per document.
4. Numbered body sections (1, 2, 3…) mirroring the paper's own structure (background/research questions → methods → findings → discussion → conclusion), each Heading 2 with a page range, subsections as Heading 3 with their own page ranges.
   **For a conceptual, review or theoretical paper, follow the paper's own argumentative structure instead** — do not force it into a methods/findings skeleton it does not have. Section the note the way the paper sections itself (e.g. diagnosis → what's missing on dimension A → what's missing on dimension B → research agenda → implications). Where such a paper stress-tests several theories in sequence, a `gridRow` table (theory / original premise / where it breaks) is usually a better rendering than several narrative paragraphs.
5. `Critical Assessment & Citable Points` (or the next number) — Key contributions / Limitations & citation caveats / Directly citable points, all as concise bullets grounded in the paper's own self-reported limitations plus the note-taker's own assessment.
...
- When the user asks for comments across a specific section ("pick 3-5 spots in this section and add comments"), distribute 3-5 across that section, each anchored to a different specific sentence/phrase.
- Always hedge clearly that this is the note-taker's own addition, e.g. "This is an addition by the note-taker, beyond the paper itself — the authors do not raise this theoretical angle…" — never blur it with what the paper itself argues.
- **Comment bodies are PLAIN TEXT — inline markers do not work in them.** `**bold**`, `*italic*` and `==highlight==` are parsed by `parseRuns()` in `docx_helpers.js`, which applies to the document BODY only; `insert_comments.py` writes comment text straight into `comments.xml` and never sees it. `scripts/insert_comments.py` strips any such markers (keeping the words) and prints a note saying how many it removed — so the output is clean either way, but the markers were still a mistake. For emphasis inside a comment, use wording ("the key point is…", "worth noting that…") or quotation marks instead.
...
- Font: `Times New Roman` for ascii/hAnsi/cs, `宋体` (SimSun) for eastAsia, on every run. This is unchanged from the Chinese-output sibling skill and stays harmless here — an all-English note has no eastAsia-range characters to trigger the fallback, but if a stray CJK term or name ever appears (e.g. quoting a Chinese-language title verbatim), it still renders correctly instead of falling back to a missing-glyph box. Also the document default color: `1A1A1A` (a near-black dark gray, not pure `000000`). `scripts/docx_helpers.js` already sets all of this up (`FONT`, `defaultStyles()`).
...
- **Captions use a non-standard label** ("Panel A", no caption at all, a caption in another language) → the scan in step 3 will miss them; skim the results section of the text version instead.
...
3. **Add real Word comments** with `scripts/insert_comments.py`. It is self-contained: if the file has no comments part yet it creates one (content-type override, relationship, and the part itself); if one already exists (e.g. scaffolded by docx-js) it appends to it. Write the comments as a small JSON file:
   ...
   ```
   **Anchor text must fall entirely within a single formatting run** — i.e. entirely inside one `**bold**` span, or entirely within a plain-text sentence with no inline markers in it. A substring straddling a bold/plain boundary won't match a single run, and the script will raise a clear error rather than silently misplacing the comment (it also errors loudly, rather than guessing, if the anchor text isn't unique in the document — pick a longer or more specific substring instead). **The `text` field is plain text**: any inline markers in it are stripped and a count is printed — write comments without markers in the first place. The default comment-author name is `"Quick-Overview Note"` (`--author` to override); the sibling Chinese skill's script defaults to a Chinese label instead — that is the one deliberate difference between the two copies of this file.
4. **Validate**:
   ```
...
7. **Verify before delivering — this step is mandatory, not optional.** A note that reads fluently but misattributes a number is worse than no note, because the error is invisible at the point of use. Go back to the source PDF and check, in this order:
   ...
   - **Author names, affiliations, journal, volume/issue, page range and DOI** in Bibliographic Information, character by character against the paper's own front matter.
   ...
   - **Every claim attributed to the paper** that is actually your own inference. If the paper does not say it, it belongs in a hedged margin comment or in the Critical Assessment section, never in the body as if the authors argued it.
...
8. **Deliver** the file to the user. If there's a way to write into the same folder as the source PDF, do so under the **same filename as the literature, plus the suffix `_EN`** (e.g. `Smith2024.pdf` → `Smith2024_EN.docx`). The suffix is not optional: the Chinese-output sibling writes `Smith2024_CN.docx` for the same paper, and without it the two notes would overwrite each other. Check first whether that folder already has an established convention — for example existing notes sitting in a dedicated subfolder rather than beside the PDFs — and follow it. If overwriting an existing note, check its current modification time first and guard the write against a conflicting concurrent edit — the user may have opened the file to leave comments since you last touched it.

## Batch mode: several papers in one request

When the user supplies more than one paper at once ("do a quick-overview note for all 10 of these"), the rule is **strictly one paper at a time, start to finish, without pausing for permission between papers.**
...
When the user attaches a docx with their own review comments (e.g. from WPS or Word), extract them structurally rather than guessing from a screenshot:
...
- If a comment's instruction is itself general ("pick 3-5 spots in this section and add comments"), that's a section-level instruction, not a single-point one — plan several new comments across that section.
...
It is a sibling of `literature-quick-review-note-cn` (the Chinese-output version):
same repository, same plugin, same scripts, different `SKILL.md` prose and one
different CLI default.
...
└── plugins/lr-note/
    ...
    ├── skills/literature-quick-review-note-cn/   — Chinese-output sibling
    └── skills/literature-quick-review-note-en/   ← this skill folder
        ├── SKILL.md                   — this file
        ...
        └── scripts/
            ├── docx_helpers.js        — reusable docx-js building blocks (fonts, colors, spacing, tables) — shared verbatim with the Chinese-output sibling
            ├── example_build.js       — complete working example in English; copy and adapt
            ├── example_comments.json  — sample comment file for the quickstart, in English
            ├── strip_highlightcs.py   — WPS compatibility fix — shared verbatim
            ├── insert_comments.py     — adds real Word margin comments, self-contained — shared except for the `--author` default
            ├── validate_docx.py       — lightweight structural sanity check — shared verbatim
            ├── test_scripts.py        — unit tests for the two OOXML scripts; run before/after editing them
            └── render_preview.sh      — .docx → PDF → JPEG-per-page, for visual spot-checking — shared verbatim
```

The pipeline has been run end to end from a clean checkout (build → strip → comment → validate → render) for this English variant. Before publishing a release, run the same workflow in Codex if Codex compatibility is part of the release claim. The scripts do not depend on an AI vendor's internal tooling; they require only the listed external runtimes and packages.
