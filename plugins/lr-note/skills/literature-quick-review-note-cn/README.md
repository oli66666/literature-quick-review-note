# Literature Quick-Overview Note (Chinese output)

A Claude/Codex-compatible skill for producing a condensed academic reading note, written in Chinese, as a WPS-compatible Word (`.docx`) file. It also includes portable Node.js and Python scripts for building, commenting, validating, and rendering the document.

For the same note written in English, see the sibling skill [`literature-quick-review-note-en`](../literature-quick-review-note-en/). Until plugin version 2.0.0 this skill was named `literature-quick-review-note`.
...
Notes are written in Chinese by design: the point is to let a Chinese-speaking researcher absorb an English paper quickly. The English-output version is a separate sibling skill, [`literature-quick-review-note-en`](../literature-quick-review-note-en/), installed by the same plugin.

When both are installed, the choice between them follows one rule, written into both skills' descriptions: an output language the user names explicitly wins ("做英文版笔记" goes to `-en` even though the request is in Chinese); otherwise the note follows the language of the request. This is Claude's judgement from the descriptions, not a hard switch, so invoke a skill by name when it must be one or the other.
